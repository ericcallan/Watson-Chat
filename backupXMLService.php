...
class XMLService {
	...
	public function buildXML() {
		$client = new Client();
		$res = $client->request('GET', '/feed.json');
		...
		foreach($data as $employee) {
			$bio = '';
			if(isset($employee->profile->bio)) {
				if(isset($employee->profile->bio->hometown) && $employee->profile->bio->hometown != '') {
					$bio = strip_tags($employee->profile->firstName) . ' is from ' . strip_tags($employee->profile->bio->hometown) . '. ';
				}
			}

			$image = '';

			if(isset($employee->profile->photos)) {
				if(isset($employee->profile->photos->thumbnailUrl)) {
					$array[$employee->profile->name] = $employee->profile->photos->thumbnailUrl;
				}
			}

			if(isset($employee->profile->firstName) && isset($employee->profile->jobTitle) && isset($employee->profile->name)) {
				$jobTitle = str_replace('&', 'and', $employee->profile->jobTitle);
				...
				$tempData = ['input' => [
						...
						'output' => [
							'prompt' => [
								'item' => strip_tags($employee->profile->name) . ' is a ' . strip_tags($jobTitle) . ' here. ' . $bio . $image,
				...
				$job = ['input' => [
						...
						'output' => [
							'prompt' => [
								'item' => strip_tags($employee->profile->name) . ' is a ' . strip_tags($jobTitle),
								'@selectionType' => 'RANDOM'
							],
						]
					]
			...
			array_push($employeeData, $tempData);
		...
		$xml->add(['flow' =>
			['folder' =>
				['output' =>
					['prompt' =>
						['item' => 'Hello, I am the the Watson Chat Bot, how can I help', '@selectionType' => 'RANDOM'],
						['goto' => [
